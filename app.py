...
if st.button("🚀 뉴스 호재 선행형 융합 분석 가동", use_container_width=True):
    ...
    # [공정 2/4] 뉴스 재료 기반 40개 기업 선제 필터링 및 네이버/FDR 연동
    with st.spinner("🧠 [공정 2/4] 뉴스 호재 분석 기반 유망 기업 40개 도출 및 정밀 시점 연산 중..."):
        
        # 1. 고속 코스피 마스터 테이블 확보하여 종목 매핑 오류 원천 차단
        try:
            kospi_master = fdr.StockListing('KOSPI')
            master_text = ""
            for _, r in kospi_master.dropna(subset=['Code', 'Name']).iterrows():
                master_text += f"{r['Name']}({r['Code']})\n"
        except:
            st.error("거래소 마스터 목록 로드 실패")
            st.stop()

        # 2. 1차 프롬프트: 28대 뉴스 흐름을 보고 호재가 살아있는 유니크한 40개 기업을 먼저 스크리닝
        prompt1 = f"""
        너는 주식 시장 전판을 읽는 퀀트 스크리너다. 아래의 [28대 대형 뉴스 데이터]의 문맥을 면밀히 분석해라.
        제공된 [코스피 거래 대상 목록] 중에서 최근 대규모 '투자, 협업, 협력, M&A, 상장, 방산 수주, AI/인공지능/로봇' 호재 재료가 발생하여 탄력이 붙은 상위 40개 기업을 선별해라.
        [주의사항] '폐지'나 리스크 악재 뉴스가 엮인 좀비 기업은 철저히 제외해라.
        출력 형식은 기호나 설명 없이 반드시 `종목명,종목코드` 형태로만 한 줄에 하나씩 딱 40줄 출력해라.

        [코스피 거래 대상 목록]
        {master_text[:2000]}

        [28대 대형 뉴스 데이터]
        {collected_macro_news[:4000]}
        """
        ...
        # 3. 40개 기업의 실시간 네이버 시세 & FDR 연휴 마감 시계열 융합 조립
        kospi_pool_text = ""
        full_aligned_list = []
        ...
        for name, ticker in scanned_stocks:
            try:
                ...
                # FDR 금융 데이터 엔진 구동 (연휴/명절/주말 자동 디텍팅 및 스킵)
                ...
                kospi_pool_text += f"{name},{ticker},현재가:{price_today},전일가:{price_yesterday}({yesterday_label}),전일대비:{change_yesterday:.2f}%,전주가:{price_last_week}({last_week_label}),전주대비:{change_last_week:.2f}%\n"
            except: pass

        # 4. 재료와 시계열 지표를 대조하여 1차 정예 후보 20개 선정
        prompt_screen20 = f"""
        너는 금융 매칭 매니저다. 아래 추출된 40개 기업의 [뉴스 재료 문맥]과 [수급/시계열 데이터]를 비교하여,
        재료가 확실하고 전일 대비 또는 주간 대비 자금 유입 탄력성이 가장 우수한 최정예 20개 종목을 최종 압축해라.
        출력 형식은 기호 없이 반드시 `종목명,종목코드` 형태로만 한 줄에 하나씩 딱 20줄 출력해라.
        ...
        st.markdown(f'<div class="timestamp-box">⏱️ 뉴스 선행 발굴 동기화 완료: 현재 기준({live_time_label}) / 연휴 돌발 스킵 방어막 가동 중</div>', unsafe_allow_html=True)
        ...
        st.markdown("### 🎯 1차 선별: 호재-시계열 융합 매칭 후보 (20개)")
        st.dataframe(pd.DataFrame(ui_table_data), use_container_width=True, hide_index=True)

    # [공정 3/4] 20개 기업 대상 심층 타겟 뉴스 매칭
    with st.spinner("📥 [공정 3/4] 20개 후보 기업의 개별 언론사 최신 뉴스 정밀 크로스 매칭 중..."):
    ...
    # [공정 4/4] 최종 압축 TOP 10 프리미엄 분석 리포트 빌드
    with st.spinner("🧠 [공정 4/4] 제미나이가 최종 유망 종목 TOP 10 압축 및 심층 퀀트 리포트 빌드 중..."):
        ...
        for name, ticker in selected_stocks:
            ...
            if m:
                quant_injection_text += f"[{name}({ticker}) 수치 데이터]: 현재가={m['p_today']}, 어제가={m['p_yesterday']}, 어제날짜={m['y_label']}, 전일등락률={m['ch_y']:.2f}%, 저번주가={m['p_lw']}, 저번주날짜={m['lw_label']}, 주간등락률={m['ch_lw']:.2f}%\n"

        prompt2 = f"""
        너는 대한민국 최고 자산운용사의 헤드 펀드매니저다. 아래 데이터를 최종 검증하여 내일 장에서 3% 이상 추가 폭발 가능성이 높은 완벽한 10개 종목을 선별하고 리포트를 작성해라.
        ...
        [데이터셋]
        {company_specific_news_text}
        {collected_macro_news[:2000]}

        ⚠️ [작성 규칙 - 절대 엄수]
        1. **HTML 태그(<div>, <p> 등)는 화면이 깨지므로 절대 사용 금지.** 100% 마크다운 서식만 사용하여 출력해라.
        2. 상승근거와 주의사항 내용 맨 앞에는 수집된 [뉴스 일자 / 출처언론사](링크)를 가공 없이 그대로 표기해라.
        3. 금액 포맷: 모든 금액 정보는 반드시 1,000단위 콤마(,)를 붙여서 원화로 명시해라. (예: 73,400원)
        4. 내일 예상가는 제공된 현재가 기준 [예상 금액]원과 ([예상상승률]%)를 직접 수식 연산해서 숫자로 찍어라.
        5. 주의 사항은 뻔한 소리를 배제하고 산업 구조, 기업 재무, 수급 동향 등 깊이 있게 파고들어 날카롭게 기술해라.
        6. 아래 제공하는 '형식 규격' 레이아웃을 공백과 글자 하나 틀리지 말고 완벽하게 일치시켜라.

        형식 규격:
        ---
        ### 📈 [순위]. 종목명 (종목코드)
        - **💰 현재 금액 ({live_time_label} 기준):** [현재가]원 / **🚀 내일 예상:** [예상 금액]원 ([예상상승률]%)
        - **⏮️ 직전 변동성:** 어제([어제날짜] 기준) 금액은 [어제가]원이었으며, 전일 대비 상승률은 [전일등락률]% 이었습니다.
        - **📅 주간 변동성:** 저번주([저번주날짜] 기준) 금액은 [저번주가]원이었으며, 주간 대비 상승률은 [주간등락률]% 이었습니다.
        ...
        **- 어제 추천 여부:** [결과]
        ...
        try:
            ...
            st.success("✨ 모든 크로스 분석 및 팩트 체크가 완벽히 완료되었습니다!")
