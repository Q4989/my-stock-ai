...
if st.button("🚀 더블 뉴스 엔진 기반 융합 분석 가동", use_container_width=True):
    ...
    # 💡 여기에 예외 처리(try-except)를 추가했습니다!
        try:
            ...
            st.error("🚨 방대한 양의 뉴스를 분석하는 과정에서 제미나이 서버에 일시적인 트래픽 과부하가 발생했습니다. 약 10초 뒤에 버튼을 다시 눌러주세요!")
            st.stop()
        ...
        2. 각 종목 분석은 아래 형식으로 깔끔하게 작성해라.
        3. 상승근거와 주의사항 앞에 반드시 [날짜 / 출처]를 표기해라.
        4. 핵심 포인트는 이모티콘과 함께 별도 강조해라.

        형식 규격:
        ---
        ### 📈 [순위] 종목명 (종목코드)
        **💰 현재 기준 금액:** [금액]원 / **🚀 내일 예상:** [상승률]%
